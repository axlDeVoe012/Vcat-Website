      ...
      <div className="relative bg-gray-900 py-20 md:py-28 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <h1 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tight">
            Our <span className="bg-gradient-to-r from-blue-400 to-blue-200 bg-clip-text text-transparent">Portfolio</span>
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project, index) => (
            <div 
              ...
              className="group cursor-pointer flex flex-col h-full bg-white rounded-[2rem] border border-gray-100 hover:border-blue-200 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/10 overflow-hidden"
            >
              {/* Image Container */}
              <div className="relative h-64 md:h-72 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 z-20">
                  <span className="bg-white/95 backdrop-blur-sm text-blue-700 text-[10px] font-black px-3 py-1 rounded-full shadow-sm uppercase tracking-[0.1em]">
                    {project.category}
                  </span>
                </div>
              </div>
              ...
              <div className="p-8 grow flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-bold text-gray-950 group-hover:bg-gradient-to-r group-hover:from-blue-700 group-hover:to-blue-400 group-hover:bg-clip-text group-hover:text-transparent transition-all duration-300">
                    {project.title}
                  </h3>
          ...
          <div className="mt-8 text-center text-white max-w-2xl">
            ...
            <div className="mt-6 flex justify-center gap-2">
              {selectedProject.images?.map((_, i) => (
                <div key={i} className={`h-1.5 rounded-full transition-all ${i === currentImgIndex ? 'w-8 bg-blue-500' : 'w-2 bg-white/20'}`} />
              ))}
            </div>
          </div>
        </div>
